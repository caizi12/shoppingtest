	           ...
	           <form id="dictForm"> 
				<table height="450px" class="inputTable" cellpadding="0" cellspacing="1"
					...
					<tr >
						...
						<td >
							<input  value=""
								 type="text" id="dictTypeSelect" name="dictTypeSelect" />
						...
						<input  type="hidden" id="dictCategory" name="dictCategory" />
						</td>
...
$.ligerDefaults.Grid.editors.maxLength=null;
 ...
 var dictTypeSelectData = [];
 <c:forEach items="${dictData.dictTypeSelect}" var="dictType" >
   dictTypeSelectData=dictTypeSelectData.concat({"dictValue": "<c:out value="${dictType.dictValue}"/>","dictName":"<c:out value="${dictType.dictName}"/>"});
 </c:forEach>
 ...
 <c:forEach items="${dictData.dictType}" var="dictType" >
   dictTypeData=dictTypeData.concat({"dictId": "<c:out value="${dictType.dictId}"/>","dictName":"<c:out value="${dictType.dictName}"/>"});
 </c:forEach>	
 ...
 $("#dictTypeSelect").ligerComboBox({
            width : 100,
            valueFieldID:"dictTypeId",
            data:dictTypeData,
...
$("#dictCategorySelect").ligerComboBox({
 ...
 { 
         ...
         columns: [
         { display: '字典ID', name: 'dictId', width:'6%', type: 'int',frozen:true },
         { display: '字典项名称', name: 'dictName',width:'12%',editor: {type: 'text' }},
         { display: '字典项值', name: 'dictValue',width:'12%',editor: {type: 'text' }},
         { display: '字典所属分类', name: 'dictTypeId', width: '12%', type: 'int', editor: { type: 'text'},
             editor: { type: 'select', data: dictTypeData, valueColumnName: 'dictId',displayColumnName:"dictName" },
             render: function (item)
             {    
                for(var i=0;i<dictTypeData.length;i++){
                 if (dictTypeData[i].dictId==item.dictTypeId)
                    return dictTypeData[i].dictName;
                 }
             }
         
          },
         { display: '字典类型', width: '6%', name: 'dictType',type:'int',
             editor: { type: 'select', data: dictTypeSelectData, readonly:true,valueColumnName: 'dictValue',displayColumnName:"dictName" },
             render: function (item)
             {    
                for(var i=0;i<dictTypeSelectData.length;i++){
                 if (dictTypeSelectData[i].dictValue==item.dictType)
                    return dictTypeSelectData[i].dictName;
                 }
             }
         },
        
         { display: '字典类别', name: 'dictCategory', width: '6%', type: 'int', 
         ...
         { display: '排序', name: 'sort', width: '5%',editor: {type: 'text' ,maxLength:10 }
           },
         ...
         url:'${ctx}/sys/dict/querySysDictList.do?dictType=1',
         width: '100%',
         delayLoad:false,
         height:'100%',
         pageSize:10,
  ...
  $("#addDict").bind("click",function(){
         dictGrid.addEditRow({dictType:1,dictCategory:1,status:1});
})
